...
L2:
lw $t7, -24($fp)  # cargar var/param i
li $t8, 5
slt $t9, $t7, $t8
beq $t9, $zero, L3  # while false -> exit
lw $t1, -24($fp)  # cargar var/param i
li $t0, -20
mul $t1, $t1, 4
add $t0, $t0, $t1
add $t0, $t0, $fp
...
lw $t5, -24($fp)  # cargar var/param i
li $t4, -20
mul $t5, $t5, 4
add $t4, $t4, $t5
add $t4, $t4, $fp
...
sub $sp, $sp, 4
sw $t4, 0($sp)
lw $t6, -32($fp)  # cargar var/param total
sub $sp, $sp, 4
sw $t6, 0($sp)
jal suma
add $sp, $sp, 8
...
lw $t9, -24($fp)  # cargar var/param i
li $t8, -20
mul $t9, $t9, 4
add $t8, $t8, $t9
add $t8, $t8, $fp
...
lw $t3, -24($fp)  # cargar var/param i
li $t2, -20
mul $t3, $t3, 4
add $t2, $t2, $t3
add $t2, $t2, $fp
...
sub $sp, $sp, 4
sw $t2, 0($sp)
lw $t4, -24($fp)  # cargar var/param i
sub $sp, $sp, 4
sw $t4, 0($sp)
jal suma
add $sp, $sp, 8
...
lw $t6, -28($fp)  # cargar var/param z
sub $sp, $sp, 4
sw $t6, 0($sp)
lw $t7, -32($fp)  # cargar var/param total
sub $sp, $sp, 4
sw $t7, 0($sp)
jal suma
add $sp, $sp, 8
...
j L5
L4:
lw $t9, -32($fp)  # cargar var/param total
li $t0, 1
add $t1, $t9, $t0
...
sub $sp, $sp, 4
sw $t2, 0($sp)
lw $t4, -36($fp)  # cargar var/param maximo
sub $sp, $sp, 4
sw $t4, 0($sp)
jal max
add $sp, $sp, 8
move $t5, $v0
sw $t5, -36($fp)  # asignar var/param maximo
lw $t6, -24($fp)  # cargar var/param i
li $t7, 1
add $t8, $t6, $t7
sw $t8, -24($fp)  # asignar var/param i
j L2
L3:
lw $t9, -36($fp)  # cargar var/param maximo
lw $t0, -32($fp)  # cargar var/param total
add $t1, $t9, $t0
